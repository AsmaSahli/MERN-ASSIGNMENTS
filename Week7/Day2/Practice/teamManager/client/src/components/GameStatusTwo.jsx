...
const GameStatusTwo = () => {
    ...
    const updateStatus = (playerId, status) => {
        ...
        axios.patch(`http://127.0.0.1:8000/api/Player/${playerId}`, { gameTwo: status })
        ...
        <TableBody>
          {
            ...
            <TableRow key={player._id}>
              ...
              <TableCell className='actions'>
                    <Button
                      className='button'
                      color={player.gameTwo === 'playing' ? 'success' : 'primary'}
                      variant={player.gameTwo==='playing'? 'contained':null}
                    ...
                    <Button
                      className='button'
                      variant={player.gameTwo==='not playing'? 'contained':null}
                      color={player.gameTwo === 'not playing' ? 'error' : 'primary'}
                      onClick={() => updateStatus(player._id, 'not playing')}
                    >
                      Not Playing
                    </Button>
                    <Button
                      variant={player.gameTwo==='undecided'? 'contained':null}
                      className='button'
